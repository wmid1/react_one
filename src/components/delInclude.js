...
class DelInclude extends Component {
  ...
  delItem =(cardId,includeId, evn) =>{
    ...
    localStorage.removeItem("commentArr_"+cardId+","+includeId);
    localStorage.removeItem("description_"+cardId+","+includeId);
    for (let i = 0; i < 20; i++) {
      localStorage.removeItem("comments_"+cardId+","+includeId+","+i);
      localStorage.removeItem("comments_"+cardId+","+includeId+","+i+"_username");
    }
