...
class DelCard extends Component {
  ...
  delItem =(cardId, columnId, evn) =>{
    let cardIdArr = JSON.parse(localStorage.getItem("array_"+columnId));
    let fakeArr = JSON.parse(localStorage.getItem("fakeArr_"+columnId));
    let cardIdArray = Array.from(cardIdArr);
